...
## グラフ用JS
$this->Html->script('plugins/narrows-master/jquery.narrows.min.js', array('inline' => false));
$js .= <<<EOF
$("#ex1-category").narrows("#ex1-menu", {
				...
				<div class="row">
					<div class="col-lg-3 col-md-3">
						<div class="form-group">
							<label for="ex1-category">代理店</label>
							<select id="ex1-category" class="form-control" name="data[TrnThanks][agency_id]">
								<option value="">全て</option>
<?php
foreach ($shops as $agency_id => $agencies) {
	$selected = $agency_id == $agencyId ? 'selected' : '';
	echo '<option value="'.$agency_id.'" '.$selected.'>'.$agencies['name'].'</option>';
}
?>
							</select>
						</div>
					...
					<div class="col-lg-3 col-md-3">
						<div class="form-group">
							<label for="ex1-menu">企業</label>
							<select id="ex1-menu" class="form-control" name="data[TrnThanks][chain_id]">
								<option value="">全て</option>
<?php
foreach ($shops as $agency_id => $agencies) {
	foreach ($agencies['data'] as $chain_id => $chains) {
		$selected = $chain_id == $chainId ? 'selected' : '';
		echo '<option value="'.$chain_id.'" data-ex1-category="'.$agency_id.'" '.$selected.'>'.$chains['name'].'</option>';
	}
}
?>
							</select>
...
foreach ($shops as $agency_id => $agencies) {
	...
	foreach ($agencies['data'] as $chain_id => $chains) {
		...
		foreach ($chains['data'] as $shop_id => $shop_name) {
			...
			$box_body .= '<div class="col-md-4 col-sm-6"><i class="'.$class.'">&nbsp;</i>'.$this->Html->Link($shop_name, '/report/shopDetail/'.$shop_id.'/'.$from.'/'.$to).'</div>';
			$shop_num++;
		}
	}
}
			...
			<div class="tab-content">
				...
				?>
					...
					<tr>
						...
						<td><?php echo $this->Html->Image(Configure::read('IMG_URL').$res2['image_url'], array('class' => 'img-responsive', 'style' => 'width: 40%;')); ?></td>
						<td><?php echo number_format($res2['nums']).'回'; ?></td>
						<td><?php echo number_format($res2['rate']).'％'; ?></td>
					</tr>
					<?php } ?>
					</tbody>
					</table>
